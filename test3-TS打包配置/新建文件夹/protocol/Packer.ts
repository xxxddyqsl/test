...
export class Packeter {
    ...
    static PackMsgReq(cmd:number, header :any, msg :any) : Uint8Array {
        let btHeader : Uint8Array;
        let headerLen : number = 0;
        let btBody : Uint8Array = new Uint8Array(0);
        let bodyLen : number = 0;
        
        let tmp = Packeter.PackData(header);
        if (tmp) {
            btHeader = tmp;
    ...
    static ParseRsp(data : Uint8Array) : ProtoEntity.TPacket {
        ...
        console.log("all len=", bts.byteLength, 
            ",cmdid=", cmdid, ",flag=", flag, ",pb header len = ",
        ...
        if (cmdid == 200) {
            let pbHeader = ProtoHelper.ParseNotifyHeader(btHeader);
            if (!pbHeader) {
            ...
            if (bodyLen > 0) {
                ...
                rsp.pb = {header: pbHeader, body: pbBody};
            } else {
                rsp.pb = {header: pbHeader, body: null};
            }
