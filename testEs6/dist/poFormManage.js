...
var Mold = function () {
    ...
    _createClass(Mold, [{
        ...
        key: "pluginSelectData",

        // 日期选择插件
        value: function pluginSelectData() {
            var that = this;
            // 获取当前时间戳 
            var date = new Date();
        ...
        value: function pluginPage() {
            ...
            * */
            new Pagination({
                ...
                currentChange: function currentChange(index, pageSize) {
                    // 页码改变时回调  TODO:第一个参数是当前页码，第二个参数是每页显示条数数量，需使用sizes第二参数才有值。
        ...
        key: "tbodyScrollbarFu",

        // tbody 是否出现滚动条 重新添加 兼容class 否则删除 兼容class
        value: function tbodyScrollbarFu() {
        ...
        // AM；格式 时间 转化
        value: function timeConvert(str) {
            // str = str.replace(/\//ig, '-');
...
function displayMutation() {
    ...
    //获取监听的dom元素
    var box = document.getElementById("tbody_content");
...
function goPoDetails(PONumber) {
    ...
    var obj = encodeURIComponent(JSON.stringify({ pONumber: PONumber }));
    var url = './PoDetails.html?obj=' + obj;
    window.open(url);
}
// 文档缺失
...
function maskLayerShow(title) {
    var arr = title != undefined && title != null ? title.replace(/\s*/g, "").split('') : [];
...
async function Download() {
    ...
    // 兼容多语言
    maskLayerShow(window.languageFn('正在拉取数据'));
    var QueryData = await mold.QueryPO();
...
function downloadAjax() {
    ...
    $.ajax({
        ...
        success: function success(res, status, xhr) {
            ...
            var filename = decodeURIComponent(xhr.getResponseHeader('Content-Disposition').split('filename=')[1]);
